...
for(const categoryUrl of categoryUrlArray){
  for (let pageNumber = from; pageNumber <= to; pageNumber++) {
    ...
    await retryPageLoad(page, `${categoryUrl}${pageNumber}`)
    // 商品一覧のリンクエレメントを取得
...
fs.writeFile('uranaiItem.json', JSON.stringify(itemLinkArray), (err) => {
  if (err) {
    console.error('ファイルに書き込む際にエラーが発生しました:', err)
  } else {
    console.log(`リンクが保存されました。`)
  }
})
...
for(const itemLink of itemLinkArray){
  ...
  try {
    await retryPageLoad(page, itemLink)
...
fs.writeFile('uranaiItemErrorLinks.json', JSON.stringify(errorLinks), (err) => {
  if (err) {
